...
export default function PostForm({ forumId }) {
  ...
  const supportedFileTypes = ['image/jpg', 'image/jpeg', 'image/png', 'video/mp4', 'image/gif'];

  const handleMediaUpload = async (e) => {
    const file = e.target.files[0];
    if (!supportedFileTypes.includes(file.type)) {
      alert("File type not supported. Please upload a JPG, JPEG, PNG, Gif or MP4 file.");
      return;
    }
    ...
    {(show) &&
      <form onSubmit={handleFormSubmit}>
        ...
        { imageLoading ? (
          ...
          image.endsWith('.jpg') || image.endsWith('.png') || image.endsWith('.jpeg') || image.endsWith('.gif') ? (
